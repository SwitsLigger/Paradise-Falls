	...
	MySQL.query('SELECT * FROM players WHERE JSON_EXTRACT(charinfo, "$.account") = @iban', {
		...
		if db ~= nil then
			...
			elseif Config.RenewedBanking then
				MySQL.query('SELECT * FROM bank_accounts_new WHERE iban = @iban', {
						['@iban'] = iban
					}, function(result2)
						local db2 = result2[1]
						local dbdata
...
QBCore.Functions.CreateCallback("okokBanking:SocietyInfo", function(source, cb, society, iban)
	...
	elseif Config.RenewedBanking then
		MySQL.query('SELECT * FROM bank_accounts_new WHERE id = @id', {
				['@id'] = society,
			}, function(result)
				local db = result[1]
				local aa
				if db ~= nil then
					if db.iban ~= nil then
						...
						MySQL.query('UPDATE bank_accounts_new SET iban = @iban WHERE id = @id', {
							['@iban'] = string.gsub(iban:upper(), " ", ""),
							['@id'] = society,
						}, function(result2)
							local db2 = result2[1]
...
AddEventHandler("okokBanking:CreateSocietyAccount", function(society, society_name, value, iban)
	...
	elseif Config.RenewedBanking then
		MySQL.query('SELECT * FROM bank_accounts_new WHERE id = @id', {
			...
			local db = result[1]
			if db == nil then
				MySQL.query('INSERT INTO bank_accounts_new (id, amount, iban) VALUES (@id, @amount, @iban)', {
					['@id'] = society,
					['@amount'] = value,
					['@iban'] = string.gsub(iban:upper(), "%s+", ""),
				}, function (result)
...
AddEventHandler("okokBanking:WithdrawMoneyToSociety", function(amount, society, societyName, societyMoney)
	...
	elseif Config.RenewedBanking then
		MySQL.Async.fetchAll('SELECT * FROM bank_accounts_new WHERE id = @id', {
			['@id'] = society,
		}, function(result)
			db = result[1]
			hasChecked = true
...
AddEventHandler("okokBanking:TransferMoneyToSocietyFromSociety", function(amount, ibanNumber, societyNameTarget, societyTarget, society, societyName, societyMoney)
	...
	if amount <= societyInfo.value then
		...
		elseif Config.RenewedBanking then
			exports['Renewed-Banking']:removeAccountMoney(society, amount)
			exports['Renewed-Banking']:addAccountMoney(societyTarget, amount)
		else
			MySQL.query('UPDATE okokbanking_societies SET value = value - @value WHERE society = @society AND society_name = @society_name', {
				['@value'] = amount,
				['@society'] = society,
